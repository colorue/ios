...
//  private let drawings = [UIImage(named: "Onboarding1"), UIImage(named: "Onboarding2"), UIImage(named: "Onboarding3"), UIImage(named: "Onboarding4"), UIImage(named: "Onboarding5")]
  ...
  private var drawings = UserDefaults.standard.stringArray(forKey: "drawings") ?? []
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    drawings = UserDefaults.standard.stringArray(forKey: "drawings") ?? []
...
//        drawingController.drawingId = drawingCell.drawingId
      }
  }
...
extension DrawingsViewController {
  ...
  override func collectionView(
    ...
    let key = drawings[indexPath.row]
    cell.drawingId = drawings[indexPath.row]
    if let retrievedImage = UserDefaults.standard.object(forKey: key) as? Data {
      cell.imageView?.image =  UIImage(data: retrievedImage)
    }
    return cell
  }
}
